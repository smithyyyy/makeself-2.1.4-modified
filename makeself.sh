...
#
# $Id: makeself.sh 1158 2007-10-04 06:18:17Z ken $
#
...
#
# 2011-03-03: 
#	  Add flag --dtpath and --dttool for customizing encryption
#	  Modified MD5_PATH by using which
#	  Add Source checksum and Library checksum for header
#
...
MS_Usage()
{
    ...
    echo "    --dtpath path   : encrypt tool path for system"
    echo "    --dttool name   : encrypt tool path for system"
    echo
...
if [ "${ENCRYPTTOOL}" = 'nil' ]; then
	...
	test -d "$archdir" || { echo "Error: $archdir does not exist."; rm -f "$tmpfile"; exit 1; }
	echo About to compress $USIZE KB of data... ENCTOOL=${ENCRYPTPATH}${ENCRYPTTOOL}
	echo Adding files to archive named \"$archname\"...
	exec 3<> "$tmpfile"
	(cd "$archdir" && ( tar $TAR_ARGS - . | eval "$GZIP_CMD" | ${ENCRYPTPATH}${ENCRYPTTOOL} -e >&3 ) ) || { echo Aborting: Archive directory not found or temporary file: "$tmpfile" could not be created.; exec 3>&-; rm -f "$tmpfile"; exit 1; }
	exec 3>&- # try to close the archive
fi
...
# store source check sum
srcchksum=`cat $archdir/hash | head -n1`

# store library source check sum
if [ -f $archdir/escrow.list ]; then
libchksum="`md5sum $archdir/escrow.list | cut -d ' ' -f 1`"
else
libchksum="00000000000000000000000000000000"
fi
